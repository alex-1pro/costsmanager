...
public class ExpenseDAOHibernate implements IExapenseDAO {
	...
	public double getTotalCost(int userId) throws ExpenseCostsManagerDAOExeption {
		/*
		 * Session session=null; double totalCost; try { session =
		 * factory.openSession(); session.beginTransaction(); String hql
		 * ="SELECT SUM(E.cost)  FROM Expense E WHERE E.userId= :user_id"; Query
		 * query=session.createQuery(hql); query.setParameter("user_id", userId);
		 * totalCost=query
		 * 
		 * 
		 * }catch (HibernateException ex) { Transaction tx = session.getTransaction();
		 * if (tx.isActive()) tx.rollback(); } finally { if (session != null)
		 * session.close(); } return expenses;
		 */
		return 0;
	}
